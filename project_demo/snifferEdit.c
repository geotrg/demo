
/*
   sniffer.c

   Example packet sniffer using the libpcap packet capture library available
   from http://www.tcpdump.org.

   ------------------------------------------

   Copyright (c) 2012 Vic Hargrave

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.
*/

#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <unistd.h>
#include <signal.h>
#include <pcap/pcap.h>
#include <netinet/tcp.h>
#include <netinet/udp.h>
#include <netinet/ip_icmp.h>
#include <ctype.h>

pcap_t* handle;
int linkhdrlen;
int packets=0;
int count = 0;
char filename[80];

FILE *f;

pcap_t* create_pcap_handle(char* device, char* filter)
{
    char errbuf[PCAP_ERRBUF_SIZE];
    pcap_t *handle = NULL;
    pcap_if_t* devices = NULL;
    struct bpf_program bpf;
    bpf_u_int32 netmask;
    bpf_u_int32 srcip;

    // If no network interface (device) is specified, get the first one.
    if (!*device) {
    	if (pcap_findalldevs(&devices, errbuf)) {
            fprintf(stderr, "pcap_findalldevs(): %s\n", errbuf);
            return NULL;
        }
        strcpy(device, devices[0].name);
    }

    // Get network device source IP address and netmask.
    if (pcap_lookupnet(device, &srcip, &netmask, errbuf) == PCAP_ERROR) {
        fprintf(stderr, "pcap_lookupnet: %s\n", errbuf);
        return NULL;
    }

    // Open the device for live capture.
    handle = pcap_open_live(device, BUFSIZ, 1, 1000, errbuf);
    if (handle == NULL) {
        fprintf(stderr, "pcap_open_live(): %s\n", errbuf);
        return NULL;
    }

    //set promiscuous mode if eq non-zero, this mode enable
    int promisc = pcap_set_promisc(handle, 1);
    if(promisc == PCAP_ERROR)
    {
	    fprintf(f, "Error set promiscuous mode %s\n", pcap_geterr(handle));
	    return NULL;
    }

    // Convert the packet filter epxression into a packet filter binary.
    if (pcap_compile(handle, &bpf, filter, 1, netmask) == PCAP_ERROR) {
        fprintf(stderr, "pcap_compile(): %s\n", pcap_geterr(handle));
        return NULL;
    }

    // Bind the packet filter to the libpcap handle.    
    if (pcap_setfilter(handle, &bpf) == PCAP_ERROR) {
        fprintf(stderr, "pcap_setfilter(): %s\n", pcap_geterr(handle));
        return NULL;
    }

    return handle;
}

void get_link_header_len(pcap_t* handle)
{
    int linktype;

    // Determine the datalink layer type.
    if ((linktype = pcap_datalink(handle)) == PCAP_ERROR) {
        fprintf(f, "pcap_datalink(): %s\n", pcap_geterr(handle));
        return;
    }

    // Set the datalink layer header size.
    switch (linktype)
    {
    case DLT_NULL:
        linkhdrlen = 4;
        break;

    case DLT_EN10MB:
        linkhdrlen = 14;  
        break;

    case DLT_SLIP:
    case DLT_PPP:
        linkhdrlen = 24;
        break;

    default:
        fprintf(f, "Unsupported datalink (%d)\n", linktype);
        linkhdrlen = 0;
    }
}

void packet_handler(u_char *user, const struct pcap_pkthdr *packethdr, const u_char *packetptr)
{
    struct ip* iphdr;
    struct icmp* icmphdr;
    struct tcphdr* tcphdr;
    struct udphdr* udphdr;
    char iphdrInfo[64];
    char srcip[64];
    char dstip[64];
    const u_char *payload;
    int size_payload, size_tcp, size_ip;
    //pcap_dumper_t *pd;

    //pd = pcap_dump_fopen(handle, f);
    //fprintf(f,"\n");
    //f=fopen(filename, "w");

    // Skip the datalink layer header and get the IP header fields.
    packetptr += linkhdrlen;
    iphdr = (struct ip*)packetptr;
    strcpy(srcip, inet_ntoa(iphdr->ip_src));
    strcpy(dstip, inet_ntoa(iphdr->ip_dst));
    sprintf(iphdrInfo, "ID:%d TOS:0x%x, TTL:%d IpLen:%d DgLen:%d",
            ntohs(iphdr->ip_id), iphdr->ip_tos, iphdr->ip_ttl,
            4*iphdr->ip_hl, ntohs(iphdr->ip_len));

    // Advance to the transport layer header then parse and display
    // the fields based on the type of hearder: tcp, udp or icmp.
    packetptr += 4*iphdr->ip_hl;
    
    switch (iphdr->ip_p)
    {
    case IPPROTO_TCP:
        tcphdr = (struct tcphdr*)packetptr;
        fprintf(f, "Protocol: TCP  %s:%d -> %s:%d\n", srcip, ntohs(tcphdr->th_sport), dstip, ntohs(tcphdr->th_dport));
        fprintf(f, "%s\n", iphdrInfo);
        fprintf(f, "%c%c%c%c%c%c Seq: 0x%x Ack: 0x%x Win: 0x%x TcpLen: %d \n",
               (tcphdr->th_flags & TH_URG ? 'U' : '*'),
               (tcphdr->th_flags & TH_ACK ? 'A' : '*'),
               (tcphdr->th_flags & TH_PUSH ? 'P' : '*'),
               (tcphdr->th_flags & TH_RST ? 'R' : '*'),
               (tcphdr->th_flags & TH_SYN ? 'S' : '*'),
               (tcphdr->th_flags & TH_FIN ? 'F' : '*'),
               ntohl(tcphdr->th_seq), ntohl(tcphdr->th_ack),
               ntohs(tcphdr->th_win), 4*tcphdr->th_off);
	/*if(IPPORT_SMTP == 25) {
		fprintf(f, "SMTP\n");
	}
	else {
		fprintf(f, "\n");
	}*/

	size_ip=4*iphdr->ip_hl;
	size_tcp=4*tcphdr->th_off;
	//if(linkhdrlen != 0) 
	payload=(u_char *)(packetptr + linkhdrlen + size_ip + size_tcp);	// define/compute payload (segment) offset

	size_payload=ntohs(iphdr->ip_len)-(size_ip+size_tcp); // compute tcp payload (segment) size
	// Print payload data; it might be binary, so don't just treat it as a string.
	if(IPPORT_SMTP == 25 && size_payload > 0) {
		fprintf(f, "  Payload (%d bytes):\n", size_payload);
		Pripayload(payload, size_payload);
	}

        fprintf(f, "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n\n");
        packets += 1;
        break;

    /*case IPPROTO_UDP:
        udphdr = (struct udphdr*)packetptr;
        fprintf(f, "Protocol: UDP  %s:%d -> %s:%d\n", srcip, ntohs(udphdr->uh_sport),
               dstip, ntohs(udphdr->uh_dport));
        fprintf(f, "%s\n", iphdrInfo);
	size_ip=4*iphdr->ip_hl;
	size_tcp=4*tcphdr->th_off;
	//if(linkhdrlen != 0)
	payload=(u_char *)(packetptr + linkhdrlen + size_ip + size_tcp);
	size_payload = ntohs(iphdr->ip_len)-(size_ip+size_tcp);
	// Print payload data; it be might be binary, so don't just treat it as a string.
	
	if(size_payload > 0) {
		fprintf(f, "  Payload (%d bytes):\n", size_payload);
		Pripayload(payload, size_payload);
	}

	fprintf(f, "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n\n");
        packets += 1;
        break;*/
 
    default:
	 fprintf(f, "Protocol: Unknown\n");
	 packets += 1;
	 return;
       
    }
}

void result_capture(int signo)
{
    struct pcap_stat stats;

    if (pcap_stats(handle, &stats) >= 0) {
        printf("%d packets captured\n", packets);
        printf("%d packets received by filter\n", stats.ps_recv); 
        printf("%d packets dropped\n\n", stats.ps_drop);
    }
}

int main(int argc, char *argv[])
{
    char device[256], filter[256]; 
    int opt;
    *device = 0;
    *filter = 0;

    // Get the command line options, if any
    while ((opt = getopt(argc, argv, "hi:n:f:")) != -1)
    {
        switch (opt)
        {
        case 'h':
            printf("usage: %s [-h] [-i interface] [-n count] [-f filename] [BPF expression]\n", argv[0]);
            exit(0);
            break;
        case 'i':
            strcpy(device, optarg);
            break;
        case 'n':
	    if (optarg == NULL) {
		    count=0;
	    }
	    else {
		    count = atoi(optarg);
	    }
            break;
	case 'f':
	    strcpy(filename, optarg);
	    break;
        }
    }

    // Get the packet capture filter expression, if any.
    for (int i = optind; i < argc; i++) {
        strcat(filter, argv[i]);
        strcat(filter, " ");
    }

    signal(SIGINT, result_capture);
    signal(SIGTERM, result_capture);
    signal(SIGQUIT, result_capture);

    // Create packet capture handle.
    handle = create_pcap_handle(device, filter);
    if (handle == NULL) {
        return -1;
    }

    // Get the type of link layer.
    get_link_header_len(handle);
    if (linkhdrlen == 0) {
        return -1;
    }

    f=fopen(filename, "w+");

    int countLoop=pcap_loop(handle, count, packet_handler, NULL);
    // Start the packet capture with a set count or continually if the count is 0.
    if (countLoop == PCAP_ERROR) {
        fprintf(stderr, "pcap_loop failed: %s\n", pcap_geterr(handle));
        return -1;
    }
    result_capture(0);
    fclose(f);
    pcap_close(handle);
    exit(0);
}
// print packet payload data (avoid printing binary data)
void Pripayload(const u_char *payload, int len) {
	int len_rem=len;
	int line_width=16;  
	int line_len;
	int offset=0; 
	const u_char *ch = payload;

	if(len<=0)
		return;

	if(len <= line_width) {
		print_hex_ascii_line(ch, len, offset);
		return;
	}
	/*data spans multiple lines */
	for (;;)
	{
		// compute current line length
		line_len=line_width % len_rem;
		// print line
		print_hex_ascii_line(ch, line_len, offset);
		// compute total remaining
		len_rem = len_rem - line_len;
		// shift pointer to remaining bytes to print
		ch = ch + line_len;
		// add offset
		offset = offset + line_width;
		// check if we have line width chars or less
		if (len_rem <= line_width)  {
			// print last line and get out 
			print_hex_ascii_line(ch, len_rem, offset);
			break;
		}
	}
	return;
}

/* print data in row of 16 bytes: offset   hex   ascii*/
void print_hex_ascii_line(const u_char *payload, int len, int offset) {
	int i, gap;
	const u_char *ch;

	//offset
	fprintf(f, "%05d  ", offset);

	// hex
	ch=payload;
	for(i=0;i<len;i++)
	{
		fprintf(f, "%02x ", *ch);
		ch++;
		// print extra space after 8th byte for visual aid
		if(i==7)
			fprintf(f, " ");
	}
	//print space to handle line less than 8 bytes
	if(len<8)
		fprintf(f, " ");
        /*if(len<4)
                fprintf(f, " ");*/

	//fill hex gap with spaces if not full line
	if(len<16) {
		gap=16-len;
		for(i=0;i<gap;i++)
		{
			fprintf(f, "   ");
		}
	}
	fprintf(f, "   ");

	//ascii (if printable)
	ch=payload;
	for(i=0;i<len;i++)
	{
		if (isprint(*ch))
			fprintf(f, "%c", *ch);
		else 
			fprintf(f, ".");
		ch++;
	}

	fprintf(f, "\n");

	return;
}




